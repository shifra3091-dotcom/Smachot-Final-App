...
const EventOwnerAuth: React.FC = () => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [currentUser, setCurrentUser] = useState<{ Success: boolean; User?: { Name?: string; Id?: number; Email?: string } } | null>(null);
  ...
  const handleLogin = async (loginDto: LoginDto) => {
    ...
    try {
      ...
      if (result.Success && result.User) {
        dispatch(setUser(result.User));
        navigate('/eventowner/all-events');
      } else {
